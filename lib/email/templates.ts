...
/* -------------------------------------------------------------------------- */
/* 1b. Verify e-mail (sent via Resend; link generated by admin.generateLink)   */
/* -------------------------------------------------------------------------- */

export function verifyEmail(input: { displayName?: string; verifyUrl: string }) {
  const subject = "Potvrďte svůj e-mail ve Vzpomínkáři";
  const firstName = input.displayName
    ? (input.displayName.split(" ")[0] ?? "").trim()
    : "";
  const hello = firstName ? `Dobrý den, ${firstName}.` : "Dobrý den.";

  const html = shell({
    title: subject,
    preheader: "Jedním kliknutím potvrdíte svůj e-mail.",
    body: `
      ${h1(hello)}
      <p style="margin:0 0 14px 0;">
        Potvrďte prosím svůj e-mail &mdash; ať víme, že vám v pořádku dorazí
        připomínky otázek i hotová kniha.
      </p>
      <p style="margin:0 0 24px 0;">Stačí kliknout na tlačítko níže.</p>
      <p style="margin:0 0 8px 0;">${cta("Potvrdit e-mail", input.verifyUrl)}</p>
      ${HR}
      <p style="margin:0;font-size:14px;color:${INK_SOFT};">
        Pokud jste o potvrzení nežádali, tento e-mail klidně ignorujte.
      </p>
      ${signoff()}
    `,
  });
  ...
  const text = [
    hello,
    "",
    "Potvrďte prosím svůj e-mail — ať víme, že vám v pořádku dorazí připomínky otázek i hotová kniha.",
    "",
    `Potvrdit e-mail: ${input.verifyUrl}`,
    "",
    "Pokud jste o potvrzení nežádali, tento e-mail klidně ignorujte.",
    "",
    "Kuba a tým Vzpomínkáře",
...
/* -------------------------------------------------------------------------- */
/* 1c. Password reset (sent via Resend; recovery link from admin.generateLink) */
/* -------------------------------------------------------------------------- */

export function passwordResetEmail(input: { resetUrl: string }) {
  const subject = "Obnovení hesla ve Vzpomínkáři";

  const html = shell({
    title: subject,
    preheader: "Odkaz pro nastavení nového hesla.",
    body: `
      ${h1("Obnovení hesla")}
      <p style="margin:0 0 14px 0;">
        Někdo požádal o nové heslo k vašemu účtu. Klikněte na tlačítko níže
        a nastavte si nové.
      </p>
      <p style="margin:0 0 8px 0;">${cta("Nastavit nové heslo", input.resetUrl)}</p>
      ${HR}
      <p style="margin:0;font-size:14px;color:${INK_SOFT};">
        Pokud jste o změnu hesla nežádali, nic se neděje — heslo zůstává beze
        změny a tento e-mail můžete ignorovat.
      </p>
      ${signoff()}
    `,
  });
  ...
  const text = [
    "Obnovení hesla",
    "",
    "Někdo požádal o nové heslo k vašemu účtu. Otevřete odkaz níže a nastavte si nové.",
    "",
    `Nastavit nové heslo: ${input.resetUrl}`,
    "",
    "Pokud jste o změnu hesla nežádali, tento e-mail můžete ignorovat.",
    "",
    "Kuba a tým Vzpomínkáře",
  ].join("\n");
